...
namespace TK.UntiyEditor
{
    ...
    [CustomEditor(typeof(PCHColliderGenerateTool))]
    public class PCHColliderGeneratorEditor : Editor
    {
        private bool isDeleteCollider;
        private bool isGenerateColliderOpenTrigger;
        private PCHColliderGenerateTool controller;
        public void OnEnable()
        {
            controller = target as PCHColliderGenerateTool;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (controller.gameObject.GetComponentsInChildren<PCHChainProcessor>() == null)
            {
                Titlebar("Tips:This tool will read physicsbone data to get fit size .", Color.grey);
            }
            if (controller.gameObject.GetComponentsInChildren<PCHColliderReader>() .Length!=0 && (controller.generateColliderList == null || controller.generateColliderList.Count == 0))
            {
