...
namespace TK
{
    ...
    public static class PCHStaticColliderFunc
    {
        ...
        public static Bounds CaculateBounds(Transform transform, PCHChainProcessor[] jointAndPointControlls)
        {
            MinMaxABBox aabb = CaculateAABB(transform, jointAndPointControlls);
            return new Bounds(aabb.Center, aabb.Extents);
        }
        public static MinMaxABBox CaculateAABB(  Transform transform,PCHChainProcessor[] jointAndPointControlls)
        {
            Transform root = transform;
            MinMaxABBox AABB;
            if (jointAndPointControlls == null|| jointAndPointControlls.Length==0)
            {
                 AABB = MinMaxABBox.CreateFromCenterAndExtents(root.position, 0);
            ...
            {
                AABB = new MinMaxABBox(jointAndPointControlls[0].RootTransform.position, 0);
                for (int i = 0; i < jointAndPointControlls.Length; i++)
                {
                    ...
                    for (int j0 = 0; j0 < fixedPoint.Count; j0++)
                    {
                        ...
                        {
                             MinMaxABBox smallAABB = MinMaxABBox.CreateFromCenterAndHalfExtents(fixedPoint[j0].transform.position, GetMaxDeep(fixedPoint[j0]));
                            smallAABB.Extents *= jointAndPointControlls[0].GetPCHSetting().structuralStretchVertical * 1.1f;
        ...
        public static List<PCHColliderReader>  GenerateBodyCollidersData(Transform  transform,List<PCHRuntimePoint> allPointTrans, bool isGenerateFinger,bool isGenerateColliderOpenTrigger,out int isGenerateSuccessful)
        {
            ...
            List<PCHColliderReader> generateColliderList = null;
        ...
        private static void GenerateCollidersData(ref List<PCHColliderReader> runtimeColliders, List<PCHRuntimePoint> allPointTrans, Animator animator, bool isGenerateFinger, bool isGenerateColliderOpenTrigger)
        {
        ...
        private static float CheckNearstPointToSegment(float MaxLength, Vector3 position, Vector3 direction, ColliderChoice choice, List<PCHRuntimePoint> pointTrans, bool isInverse = false)
        {
        ...
        private static PCHColliderReader CreateUntiyBoxCollider(Vector3 position, Quaternion rotation, Vector3 size, ColliderChoice colliderChoice, Transform appendTransform, string name, bool isGenerateColliderOpenTrigger = true, CollideFunc collideFunc = CollideFunc.OutsideLimit, bool isOwner = false)
        {
            ...
            PCHColliderReader target = transform.gameObject.AddComponent<PCHColliderReader>();
            target.collideFunc = collideFunc;
            target.colliderMask = colliderChoice;
            return target;
        }
        private static PCHColliderReader CreateUntiyCapsuleCollider(float radius, Vector3 pointHead, Vector3 pointTail, ColliderChoice colliderChoice, Transform appendTransform, string name, bool isGenerateColliderOpenTrigger = true, CollideFunc collideFunc = CollideFunc.OutsideLimit, bool isOwner = false)
        {
        ...
        private static PCHColliderReader CreateUnitySphereCollider(float radius, Vector3 position, ColliderChoice colliderChoice, Transform appendTransform, string name, bool isGenerateColliderOpenTrigger = true, CollideFunc collideFunc = CollideFunc.OutsideLimit, bool isOwner = false)
        {
