...
namespace TK.Mono
{
    ...
    public class PCHRuntimeController : MonoBehaviour, IPCHPhysicMonoComponent
    {
        ...
        public List<PCHColliderReader> overlapsColliderList;
        public Bounds OverlapBox;


        public UpdateMode updateMode=UpdateMode.FixedUpdate;
        public PCHChainProcessor[] allChain;
        private PCHPhysicsKernel ADBkernel;
        ...
        private void Start()
        {
            ...
            ADBkernel = new PCHPhysicsKernel();
        ...
        private void Run(float inputDeltaTime)
        {
            ...
            addForce += PCHWindZone.getaddForceForce(transform.position) * windForceScale * deltaTime;
        ...
        public void UpdateOverlapsCollider()
        {
            ...
            {
                if (PCHColliderReader.ColliderTokenDic.TryGetValue(colliders[i].GetInstanceID(),out PCHColliderReader colliderToken))
                {
        ...
        public void SetPhysicData(PCHChainProcessor[] jointAndPointControlls)
        {
        ...
        {
            var parentRuntimeController = transform.parent?.GetComponentInParent<PCHRuntimeController>();
        ...
        public void ListCheck()
        {
            ...
            {
                overlapsColliderList = new List<PCHColliderReader>();
            }
            if (tempColliderReads==null)
            {
                tempColliderReads = new List<ColliderRead>(); 
            }
            allChain = gameObject.GetComponentsInChildren<PCHChainProcessor>();
            overlapsColliderList.Clear();
            overlapsColliderList.AddRange(gameObject.GetComponentsInChildren<PCHColliderReader>());
        }
        ...
        public bool GetConstraintByKey(string key, ConstraintType constraintType, out PCHRuntimeConstraint[] returnConstraint)
        {
            List<PCHRuntimeConstraint> constraints = new List<PCHRuntimeConstraint>();
            bool isFind = false;
            for (int i = 0; i < allChain.Length; i++)
            {
